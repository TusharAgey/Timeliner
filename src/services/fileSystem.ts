...
type FileSystemHandlePermissionDescriptorLike = {
...
export const verifyPermission = async (handle: PermissionCapableHandle) => {
  const options: FileSystemHandlePermissionDescriptorLike = {
    mode: "readwrite",
  };
...
export const createWorkspaceScaffold = async (
  ...
  await Promise.all(
    data.projects.map((project) =>
      ensureFile(projectsDir, `${project.slug}.json`, project),
    ),
  );
};

export const loadWorkspaceFromHandle = async (
  handle: WorkspaceFolderHandle,
  ...
  const labels = await readJson<Label[]>(
    lookupsDir,
    "labels.json",
    labelSchema.array(),
  );
...
export const saveWorkspaceToHandle = async (
  handle: WorkspaceFolderHandle,
  data: WorkspaceData,
) => {
  const projectsDir = await handle.getDirectoryHandle("projects", {
  ...
  await Promise.all(
    data.projects.map((project) =>
      writeJson(projectsDir, `${project.slug}.json`, project),
    ),
  );
};
