...
export const fullDate = (value: string) =>
  format(parseDate(value), "MMM d, yyyy");
export const dayOffset = (value: string, from = today()) =>
  differenceInCalendarDays(parseDate(value), from);
export const betweenDays = (start: string, end: string) =>
  eachDayOfInterval({ start: parseDate(start), end: parseDate(end) });
